...
def _build_sucursales_where(sucursales_permitidas, alias='c'):
    """Construye filtro WHERE para RBAC por sucursales permitidas.
    Retorna string SQL o None si no aplica."""
    if sucursales_permitidas is None:
        return None
    if len(sucursales_permitidas) == 0:
        return f"{alias}.id_sucursal IN (-1)"  # sin acceso
...
def cargar_info_cliente(id_cliente, sucursales_permitidas=None):
    """Obtiene datos maestros de un cliente desde dim_cliente."""
    suc_filter = _build_sucursales_where(sucursales_permitidas)
    extra_where = f"AND {suc_filter}" if suc_filter else ""
    query = f"""
        SELECT
            c.id_cliente,
            c.razon_social,
